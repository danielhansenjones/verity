...
def main():
    # Schema bootstrap is handled out-of-band by scripts/init_db.py. The
    # worker process assumes the schema exists by the time it starts.
        ...
        try:
            with get_session() as db:
                ...
                if job is None:
                    ...
                    # Idempotency guard: a prior attempt may have committed
                    # COMPLETED before its ack reached Redis, or the entry may
                    # have been reclaimed after completion. Treat as no-op
                    # success rather than re-running stages (which would
                    # duplicate assembly output / double-count).
                    if (
                        job.status == JobStatus.COMPLETED
                        or job.stage == JobStage.DONE
                    ):
                        log.info(
                        ...
                        try:
                            if job.stage == JobStage.INGESTION:
                                ...
                                with job_stage_duration_seconds.labels(
                                    ...
                                    ingestion.run(
                                        job, db, storage, embedding_model
                                    )
                            ...
                            if job.stage == JobStage.ASSEMBLY:
                                ...
                                with job_stage_duration_seconds.labels(
                                    ...
                                    if scored is None:
                                        ...
                                        scored = scorer.score_chunks(
                                            chunks, tone_pipeline
                                        )
